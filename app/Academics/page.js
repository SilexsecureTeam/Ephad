...
const gallerySections = [
  {
    title: "Classrooms",
    images: [Classroom, SocialStudies, Classroom, SocialStudies, Classroom],
    description:
  ...
  {
    title: "Library",
    images: [SocialStudies, Classroom, SocialStudies, Classroom, SocialStudies],
    description:
    ...
    <div className="relative border-2 border-black shadow-lg shadow-black rounded-xl w-full h-auto py-8 md:py-10">
      <div className="relative overflow-hidden h-[200px] sm:h-[250px] md:h-[250px]">
        ...
        <div className="relative w-full h-full">
          {section.images
            .slice(slideIndexes[sectionIndex], slideIndexes[sectionIndex] + getVisibleImages())
            .map((image, imgIndex) => (
              <div
                key={imgIndex + slideIndexes[sectionIndex]} // Adjust key to account for shifting images
                className="absolute top-0"
                style={{
                  left: `${imgIndex * (100 / getVisibleImages())}%`,
                  width: `${100 / getVisibleImages()}%`,
                  transition: "left 0.3s ease-in-out",
                }}
              >
                <Image
                  src={image}
                  alt={`${section.title} image ${imgIndex + slideIndexes[sectionIndex] + 1}`}
                  className="absolute top-[-40px] sm:top-[-50px] md:top-[-60px] w-full h-[200px] sm:h-[250px] md:h-[250px] border-2 border-black rounded-xl object-cover px-2"
                />
              </div>
            ))}
        </div>
